...
| maxSellLossPercent | `0` (запрет sell в убыток через midpoint-механизм) |
...
**Rebalance:** если `|currentPrice - gridCenterPrice| / gridCenterPrice > rebalancePercent` (2%) и cooldown 5 мин прошёл:
- **Drift UP:** `cancelAllPreserveCounterSellMeta` — отменяет все ордера, сохраняет counter-sell anchors (`oldBreakEven`, `originalPlannedSellPrice`), сбрасывает runtime halving. Грид пересобирается с нуля вокруг новой цены.
- **Drift DOWN (split rebalance):** `cancelBuySide` — отменяет **только buy**, sell-ы остаются на месте. Затем `initCounterSellTrailing(currentPrice)` **активирует halving** на всех counter-sells с anchors (см. "Halving" ниже).

**Force-rebalance** (`setGridCenterPrice(sym, 0)` → на след. тике `initGrid` попадает в ветку `gridCenter <= 0`):

Триггеры force-rebalance (6 мест в коде):
1. **Startup при пустом center** — первый запуск / миграция старого state
2. **Auto-spacing изменил spacing** ([combo-manager.ts:252](bybit-combo-bot/src/strategies/combo-manager.ts#L252)) — при `autoSpacingPriority: "auto"` для каждой изменённой пары
3. **Telegram `/unsellgrid BASE`** — выход из sellgrid-режима
4. **Telegram `/unfreezebuy BASE`** — разморозка после freezebuy
...
Отличие force-rebalance от split rebalance DOWN: force всегда вызывает `cancelAllPreserveCounterSellMeta` (сбрасывает runtime halving), НЕ активирует halving. Halving запускается **только** через split rebalance DOWN.
...
Общие константы: `breakEvenMult = 1 + minSellProfitPercent/100` (1.005 = avgEntry × 1.005), `lossMult = 1 - maxSellLossPercent/100` (при `maxSellLossPercent=0` → lossMult=1.0). `minSellPrice = avgEntry × breakEvenMult`. `lossThreshold = avgEntry × lossMult`.

1. **Grid Sell Guard** ([grid.ts:536-551](src/strategies/grid.ts#L536-L551), при placeGridOrders): если `level.price < minSellPrice`:
   - Если `minSellPrice ≤ currentPrice` → skip `below entry` (цена рынка уже выше break-even, ждём)
   - Иначе `midpoint = (level.price + minSellPrice) / 2`; если `midpoint > lossThreshold` → sell ставится на midpoint, иначе → skip `midpoint >N% loss`
   - При `maxSellLossPercent: 0` второе условие становится `midpoint > avgEntry` → sell размещается ТОЛЬКО выше цены входа

2. **Counter-Sell Guard** ([grid.ts:780-800](src/strategies/grid.ts#L780-L800), при flip buy→counter-sell): идентичная логика с теми же константами. При skip пишется `Counter-sell skipped: midpoint ... Relying on orphan-sell / existing higher sells` — позиция останется покрытой через orphan-sell на след. тике или уже существующими sells выше.
...
4. **Counter-Sell Midpoint Halving** (runtime trailing после split rebalance DOWN). Metadata заполняется в 2 этапа:
   - **Phase 1 — anchors** (при flip buy→counter-sell в `sync.ts` или `grid.ts`): ставятся `oldBreakEven = buyFillPrice × 1.005` и `originalPlannedSellPrice = counterPrice`. Живут весь жизненный цикл ордера.
   - **Phase 2 — runtime** (`initCounterSellTrailing`, вызывается только из split rebalance DOWN): если `level.price > virtualNewSellPrice = currentPrice × (1 + sellSpacing/100)` — заполняются `virtualNewSellPrice` (цель halving) + `nextStepAt = Date.now() + counterSellTrailStepHours × 3600000`. Шаг 1 делается сразу:
     - Если `virtualNewSellPrice < oldBreakEven` (защита включена): `newPrice = max(oldBreakEven, midpoint(originalPlannedSellPrice, virtualNewSellPrice))` → лог `protected halving`
     - Иначе (`virtualNewSellPrice ≥ oldBreakEven`): сразу `newPrice = virtualNewSellPrice`, runtime-поля обнуляются → лог `direct`
   - **Последующие шаги** (на каждом тике): если `Date.now() >= nextStepAt` → `newPrice = max(oldBreakEven, midpoint(level.price, virtualNewSellPrice))`, `nextStepAt += stepMs`. Халвинг никогда не ставит ниже `oldBreakEven`.
   - **Halving-таймер переживает рестарт** — `nextStepAt` хранится в `bot-state.json` как абсолютный Unix-timestamp. После рестарта сравнение `Date.now() >= nextStepAt` засчитывает downtime.
   - **Force-rebalance сбрасывает runtime** (`cancelAllPreserveCounterSellMeta` обнуляет `virtualNewSellPrice` и `nextStepAt`), но сохраняет anchors. Halving запустится заново при следующем split rebalance DOWN.
   - **counterSellTrailStepHours:** `>0` — halving шагами; `0` — снап сразу на virtualNewSellPrice без шагов; `<0` — trailing полностью выключен.

5. **Orphan-sell** (непокрытая free-balance позиция, [grid.ts:1005-1013](src/strategies/grid.ts#L1005-L1013)):
   ```
   rawEffectivePct = sellSpacingPct × priceStep            // priceStep = 1, 2, 3 ...
   effectivePct    = max(rawEffectivePct, minSellProfitPercent)   // FLOOR от комиссий
   sellPrice       = max(avgEntry, currentPrice) × (1 + effectivePct / 100)
   ```
   `priceStep` начинается с 1, увеличивается на +1 при каждом новом orphan-sell в тике (ladder размещения до `orphanSellMaxPerTick`) или при коллизии с существующим sell. **FLOOR** через `minSellProfitPercent` (0.5%) добавлен в последнем коммите — защищает от размещения ниже break-even при очень маленьком sellSpacing (например при low-volatility auto-spacing). При срабатывании FLOOR пишется INFO-лог `orphan-sell floor activated: ...`.
...
**Итог:** при текущих настройках (`maxSellLossPercent: 0`, orphan-sell floor, `useEmaFilter: true`) грид **никогда** не размещает ordеr ниже `avgEntryPrice × 1.005`. Единственный механизм, способный автоматически продать в убыток — Stop-Loss при -20% от entry (глобальная защита).
